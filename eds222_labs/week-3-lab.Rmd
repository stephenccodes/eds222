...
source(here("eds222_labs", "_common.R"))
...
**The water temperature model fits the data better because the approximate slope of the data matches the R\^2 value. The graph for this variable is pretty steep and the value we got was 0.92**

```{r}
size_lat <- lm(size~latitude, pie_crab)

water_lat <- lm(water_temp~latitude, pie_crab)
...
# SSE size
see <- sum(resid(size_lat)^2)

# SSE water
temp_resid <- sum(resid(water_lat)^2)
```
...
#mpg$year <- as.factor(mpg$year)mpg %>%
mpg <- mpg %>%
  mutate("year_fct" = as.factor(mpg$year))
...
```{r}
ggplot(mpg_fct, aes(x = displ, y = hwy)) +
  geom_point(aes(col = year_fct))
```
...
ggplot(mpg, aes(displ, hwy, color = mpg_fct)) +
  geom_point() +
